...
class Environment:
    def __init__(self, dimensions, cell_type):
        self.grid = [[Cell(i, j) for j in range(dimensions)] for i in range(dimensions)]
        self.target_cell = None
        print("The terrain type is {}".format(cell_type))
        self.set_values(cell_type)

    def set_values(self, cell_type):
        ...
        while cell_type != self.grid[row][col].type:
            row = random.randrange(len(self.grid))
            col = random.randrange(len(self.grid))

        self.grid[row][col].target = TARGET_FLAG
        print("Target is at : ({0} , {1}) {2}".format(row, col, cell_type))
